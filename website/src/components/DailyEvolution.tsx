...
export const DailyEvolution: React.FC = () => {
  ...
  return (
    <div className="bg-[#111111] border border-[#222222] p-6 rounded-xl overflow-hidden h-full flex flex-col text-white">
      ...
      <div className="flex items-stretch justify-between gap-2 h-48 mb-2">
        {evolutionData.map((data, index) => (
          <div key={data.day} className="flex-1 flex flex-col items-center gap-3 group">
            <div className="relative w-full flex justify-center items-end flex-1">
              {/* Tooltip */}
              <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-brand text-white text-[10px] font-bold py-1.5 px-2.5 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none transform translate-y-2 group-hover:translate-y-0 z-10">
                {data.usage}% Usage
              ...
              {/* Bar */}
              <div
                className="w-full max-w-[32px] bg-brand/20 border border-brand/30 rounded-t-md transition-all duration-700 group-hover:bg-brand/40 group-hover:border-brand/50 relative"
                style={{
