...
export const DailyEvolution: React.FC = () => {
  ...
  const peakDay = evolutionData.find(d => d.usage === maxUsage);
  const avgEfficiency = (evolutionData.reduce((acc, curr) => acc + curr.usage, 0) / evolutionData.length).toFixed(1);

  const weeklyGrowth = evolutionData.length > 1
      ...
      <div className="mt-8 pt-6 border-t border-[#222222] grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <div className="text-sm font-semibold">
            {peakDay ? `${peakDay.day === 'Mon' ? 'Monday' :
                        peakDay.day === 'Tue' ? 'Tuesday' :
                        peakDay.day === 'Wed' ? 'Wednesday' :
                        peakDay.day === 'Thu' ? 'Thursday' :
                        peakDay.day === 'Fri' ? 'Friday' :
                        peakDay.day === 'Sat' ? 'Saturday' :
                        peakDay.day === 'Sun' ? 'Sunday' : peakDay.day} @ ${peakDay.usage}%` : 'N/A'}
          </div>
        </div>
