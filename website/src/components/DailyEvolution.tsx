...
export const DailyEvolution: React.FC = () => {
  const { maxUsage, peakDay, avgEfficiency, weeklyGrowth } = useMemo(() => {
    if (!evolutionData || evolutionData.length === 0) {
      return { maxUsage: 0, peakDay: null, avgEfficiency: '0.0', weeklyGrowth: 0 };
    }
    ...
    const firstUsage = evolutionData[0].usage;
    const growth = evolutionData.length > 1 && firstUsage !== 0
      ? Math.round(((evolutionData[evolutionData.length - 1].usage - firstUsage) / firstUsage) * 100)
      : 0;

    return { maxUsage: max, peakDay: peak, avgEfficiency: avg, weeklyGrowth: growth };
  }, []);

  return (
    <div className="bg-[#111111] border border-[#222222] p-6 rounded-xl overflow-hidden h-full flex flex-col text-white">
      ...
      <div className="flex items-stretch justify-between gap-2 h-48 mb-2 flex-grow">
        {evolutionData.map((data, index) => (
          <div key={data.day} className="flex-1 flex flex-col items-center gap-3 group">
            <div className="relative w-full flex-1 flex justify-center items-end h-full">
              {/* Tooltip */}
              <div className="absolute -top-11 left-1/2 -translate-x-1/2 bg-brand text-white text-[10px] font-bold py-1.5 px-2.5 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none transform translate-y-2 group-hover:translate-y-0 z-10 whitespace-nowrap">
                {data.usage}% Usage
              ...
              <div
                ...
                style={{
                  height: maxUsage > 0 ? `${(data.usage / maxUsage) * 100}%` : '0%',
